...
-- The names, areas, and populations of cities in New England -- Connecticut (CT), 
-- Maine (ME), Massachusetts (MA), New Hampshire (NH), Rhode Island (RI) and Vermont (VT)
SELECT city_name, area, population
FROM city
WHERE state_abbreviation IN ('MA','CT', 'ME', 'NH', 'RI', 'VT');

-- Or for states not in New England
SELECT city_name, area, population
FROM city
WHERE state_abbreviation NOT IN ('MA','CT', 'ME', 'NH', 'RI', 'VT');
...
-- LIKE is case sensitive, ILIKE is a postgres specific case insensitive version of LIKE
SELECT city_name
FROM city
...
-- Can use dates with other operators like BETWEEN
-- Can use either the mm/dd/yyyy  or   yyyy-mm-dd
SELECT *
FROM park
WHERE date_established BETWEEN '1/1/1900' AND '1950-01-01';
