...
public class Exercise01_StoreOrders {
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        int totalRevenue = 0;
        int costOfSmallCheese = 8;
        int costOfMediumCheese = 11;
        int costOfLargeCheese = 14;
        for(int i = 0; i<orders.length; i++)
        {
            if(orders[i] == SMALL_CHEESE ){
                totalRevenue += costOfSmallCheese;
            }
            else if(orders[i] == MEDIUM_CHEESE){
            totalRevenue += costOfMediumCheese;
        }
            else if(orders[i] == LARGE_CHEESE )
                totalRevenue += costOfLargeCheese;
        }
        return totalRevenue;
    }
}
